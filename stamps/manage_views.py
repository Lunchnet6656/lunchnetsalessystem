...
# ============ 食数予測（直近の営業日・廃棄削減の仕込み目安） ============
# 仕様: 直近2〜3営業日ぶんの販売数(=食数)を店舗別に予測する。仕込み量の目安に使う。
# モデル: 店舗×曜日の「同曜日・直近4件の販売数を新しいほど重く加重平均」。
#   実データ220日でバックテストした結果 MAE≈4.7食 / MAPE≈10%（naive比で改善）で採用。
#   予測対象は total_sales_quantity（実売＝食数）。売れ残り(total_remaining)も併記して仕込み判断に使う。
...
def _predict_samewd(same_sold, k=FORECAST_SAMEWD_K):
    """同曜日の販売数リスト（古→新）から予測値を返す。"""
    return _weighted_recent(same_sold[-k:])


def _meal_forecast(today, days_ahead=FORECAST_DAYS_AHEAD):
    """直近の営業日ぶんの食数（販売数）を店舗別に予測して返す。

    併せて直近28日でのバックテスト誤差（MAE/MAPE）も計算し、精度の目安として同梱する。
    """
    ...
    for td in target_dates:
        ...
        for no in active:
            ...
            if len(same) < FORECAST_MIN_SAMEWD:
                continue
            pred = _predict_samewd([s for _, s, _ in same])
            ...
            locs.append({
                "no": no, "name": name_map.get(no, f"No{no}"),
                "pred": int(pred + 0.5),
                "avg_rem": round(avg_rem, 1),
                "last3": last3, "samples": len(same),
            })
        locs.sort(key=lambda x: -x["pred"])
        forecast_days.append({
            "date": td, "weekday": WEEKDAY_JA[wd],
...
def forecast(request):
    ...
    return render(request, "stamps/manage/forecast.html", {
        "nav": "forecast",
        "today": today,
        "fc": fc,
    })
